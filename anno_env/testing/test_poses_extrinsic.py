...
from utils.camera_utils import load_frame_intrinsics, load_frame_distortions, load_extrinsics
...
def main():
    ...
    while True:
        for frame_id in frameids:
            ...
            for (rvec, tvec), frame in zip(rvecs_and_tvecs, frames):
                cv2.aruco.drawAxis(frame, camera_intrinsics_dict[frame_id], camera_distortions_dict[frame_id], rvec, tvec / 9, 0.01)  # Draw Axis
    ...
    for frame_id in frameids:
        ...
        aruco_pose = calculate_aruco_from_bgr_and_depth(frame, depth, cam_scale, camera_intrinsics_dict[frame_id], camera_distortions_dict[frame_id], aruco_dict, parameters)
