...
def normalize_vector(v, return_mag =False):
    batch=v.shape[0]
    n = v.shape[1]
    v_mag = torch.sqrt(v.pow(2).sum(2))# batch*n
    v_mag = torch.max(v_mag, torch.autograd.Variable(torch.FloatTensor([1e-8]).cuda()))
    v_mag = v_mag.view(batch,n,1).expand(batch,n,v.shape[2])
...
def cross_product( u, v):
    ...
    out = torch.cat((i.view(batch,n,1), j.view(batch,n,1), k.view(batch,n,1)),dim=2)#batch*n*3
...
def compute_rotation_matrix_from_ortho6d(ortho6d):
    x_raw = ortho6d[:,:,0:3]#batch*n*3
    y_raw = ortho6d[:,:,3:6]#batch*n*3

    x = normalize_vector(x_raw) #batch*n*3
    z = cross_product(x,y_raw) #batch*n*3
    z = normalize_vector(z)#batch*n*3
    y = cross_product(z,x)#batch*n*3

        
    x = x.view(-1,3,1)
    y = y.view(-1,3,1)
    z = z.view(-1,3,1)
    matrix = torch.cat((x,y,z), dim=2) #(batch*n)*3*3
    return matrix
