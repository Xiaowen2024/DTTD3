...
class CameraPoseCleaner():
    ...
    def clean_camera_pose_file(scene_dir, write_cleaned_to_file=False):
        ...
        if not headers:
            raise ValueError("Headers list is empty. Please check the input file.")

        marker_columns = [i for (i, h) in enumerate(headers) if "Marker" in h]

        if len(marker_columns) > 0:  # Check if marker_columns is empty
            first_marker_column = min(marker_columns)
